...
- **flake.nix** — Entry point. Single input (nixpkgs-unstable), single output (`nixosConfigurations.nixos`). Imports `configuration.nix`.
- **configuration.nix** — All system config: packages, services, users, desktop environment, locale, audio, etc.
